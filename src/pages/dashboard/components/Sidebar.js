...
const Sidebar = ({ classes, staticContext, location, ...rest }) => {
  ...
  useEffect(() => {
    //TODO: set correct logic for loadGuard
    // let routeDecrement = 0;
    // routes.forEach((route, routeIndex) => {
    //   if (route.children) {
    //     let childDecrement = 0;
    //     route.children.forEach((child, childIndex) => {
    //       console.log("[child]", child);
    //       if (!access.child.id) {
    //         route.children.splice(childIndex - childDecrement, 1);
    //         childDecrement++;
    //       }
    //     });
    //   }
    ...
    //   if (!access.route.id) {
    //     routes.splice(routeIndex - routeDecrement, 1);
    //     routeDecrement++;
    //   }
    // });
  }, [access]);
  ...
  return (
    <Drawer variant="permanent" {...rest}>
      <Brand component={NavLink} to="/" button>
        <BrandIcon />{" "}
        <Box ml={1}>
          {/* Dolphin Chat <BrandChip label="PRO" /> */}
          DolphinChat
        </Box>
      </Brand>
      <Scrollbar>
